    ...
    protected void update() {
        ...
        if (playerDelay % 10 == 0) {
	    	...
	    	switch (playerDirection) {
				case LEFT: player.setX(player.getX() - 30.0); break;
				case RIGHT: player.setX(player.getX() + 30.0); break;
	        	case UP: player.setY(player.getY() - 30.0); break;
				case DOWN: player.setY(player.getY() + 30.0); break;
	    	}
			if(trail.isCollision(player.getX(), player.getY()) || player.getX() == 0 || player.getX() == 600 || player.getY() == 0 || player.getY() == 390) { //|| trail.touchBorder(getWidth(), getHeight())
